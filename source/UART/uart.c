/***************************************************************************/ /**
  @file     uart_DRV.c
  @brief    UART driver for Kinetis K64F. Supports UART0 (microUSB) and UART3 (on-board).
  @author   Team 1
 ******************************************************************************/
 ...
 ******************************************************************************/
static void printString(UARTx_t uart, char *str);
...
static uint8_t rxChar[NUM_UART_CHANNELS];
...
void UARTsendChar(UARTx_t id, char data)
{
    ...
    else if (UART_QueueStatus() > 0 && UART_QueueStatus() < QOVERFLOW - 1)
    {
        UART_PushQueue(data);
    }
}
